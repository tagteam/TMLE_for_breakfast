run_ltmle_ews <- function(name_outcome,
                          time_horizon,
                          time_interval_width_months = "NA",
                          subset=NULL,
                          subsamplesize=NULL,
                          test=FALSE,
                          regimen_data,
    ...
    result <- foreach(tk=time_horizon,.combine="rbind")%do%{
        loop <- foreach(REG = names(regimen_data))%do%{
            ## print(REG)
            bsl_covariates <- baseline_data ## Removed selection of baseline variables
            ...
            pl_info=prepare_data(regimen_data=regimen_data,outcome_data=outcome_data,
                                 ...
                                 order_YC=FALSE,deterministic.Q.function=det.Q.function,abar=rep(1,tk))
            ...
            if (sum(this <- sapply(pl$data,function(x)length(unique(x))==1))>0){
                ...
                pl_info=prepare_data(regimen_data=regimen_data,
