...
get_rhs <- function(timepoint, name_baseline_covariates, name_time_covariates, name_regimen, regimen = TRUE,
                    Markov = NULL,
  ...
  if(length(name_time_covariates[name_time_covariates%in%Markov])>0) {
      form = paste0(form, ifelse(length(form)>0," + ", ""),
                    paste(setdiff(sapply(name_time_covariates[name_time_covariates%in%Markov],
  ...
  if(regimen == TRUE) {
      form = paste0(form, ifelse(length(form)>0," + ", ""),
                   paste(setdiff(sapply(name_regimen, function(nt) {paste0(nt, "_", 0:max(0, (timepoint - 1)))}),
                                 constant_variables), collapse = " + "))
  }
  form[]
}
