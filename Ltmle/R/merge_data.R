merge_data <- function(time_horizon,
  ...
  if(order_YC == TRUE){
    work_data = work_data[,c("pnr", name_baseline_covariates,unlist(sapply(time_grid, function(timepoint){
      if(timepoint == 0){
        paste0(c(name_time_covariates,regimen_name),"_",timepoint)
      } else{
        if(timepoint != time_grid[K]){
          paste0(c(name_outcome, name_censoring, name_time_covariates, name_comp.event, regimen_name),"_",timepoint)
        } else {
          paste0(c(name_outcome, name_censoring),"_",timepoint)
        }
      }
    }))), with = FALSE]
  } else {
    work_data = work_data[,c("pnr", name_baseline_covariates,unlist(sapply(time_grid, function(timepoint){
      if(timepoint == 0){
        ...
        if(timepoint != time_grid[K]){
          paste0(c(name_censoring, name_outcome, name_time_covariates, name_comp.event, regimen_name),"_",timepoint)
        } else {
