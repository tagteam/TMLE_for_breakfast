## Get simulated data from lava model and sample size as cleaned data table

get_simdata <- function(lava_model, sample_size){
  simdata = as.data.table(sim(lava_model, sample_size))
  simdata[, pnr := c(1:simdata[,.N])] ## Include pnr in simdata
  
  Y_nodes_position = match(paste0(param$outcome,"_",1:param$time_horizon), names(simdata))
  C_nodes_position = match(paste0(param$censoring,"_",1:param$time_horizon), names(simdata))
  D_nodes_position = match(paste0(param$comp.event,"_",1:(param$time_horizon-1)), names(simdata))
  ...
  for(k in Y_nodes_position){
    ...
    if(any(has_outcome <- (simdata[[k]]%in%1))){
      for(l in later_D_nodes) {set(simdata,j=l,i=which(has_outcome),value=0)}
  ...
  if(length(param$censoring)>0){ ## Make sure that no event happens after death
    for(k in C_nodes_position){
      ...
      if(any(is_censored <- (simdata[[k]]%in%0))){
        ...
        for(l in later_D_nodes) {set(simdata,j=l,i=which(is_censored),value=0)}
