...
#+BEGIN_SRC R  :results silent :exports code  :session *R* :cache yes :eval (never-plain-export)  
library(data.table)
library(lava)
...
#+BEGIN_SRC R  :results output drawer  :exports code  :session *R* :cache yes :eval (never-plain-export)
set.seed(20230523)
...
*** Hint:
#+BEGIN_SRC R  :results output raw  :exports code  :session *R* :cache no  :eval (never-plain-export)
...
#+BEGIN_SRC R  :results silent  :exports code  :session *R* :cache no    :eval (never-plain-export)
fit10.tmle$fit$g
#+END_SRC
