...
tar_option_set(
  ...
  format = "rds", # default storage format
    controller=crew_controller_local(workers=30, garbage_collection = TRUE) #, seconds_idle = 320
)
...
## Import large datesets ----
  ...
  tar_target(dt_lab, get_lab(pnrlist=pnrlist,
                           ...
                           hba.lower=0.19,hba.upper=121),cue=import_mode),
  #Import LPR
...
## Exclusions ----
  ...
  tar_target(exclusions_DEVOTE,exclusions.devoteFUN(dt_index=dt_index,
                                             ...
                                             sexage=sexage,
                                             maxyears=20,
                                             hbalim=53),cue=manage_mode),


  #LTMLE data preparation 
...
# Treatment regime wide LTMLE prep ----
...
# Baseline wide with all covariates LTMLE prep
...
tar_target(names_baseline_covariates,
           c("pnr","age","sex",
             "incomegroup", "edu",  # <- When income is included continously, minimizer fails at time 9 for dpp4 and mace
             # "urbanization","edu", #<- one of these!! 17apr24
